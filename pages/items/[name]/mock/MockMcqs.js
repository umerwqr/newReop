...
export default function Quiz() {
  ...
  console.log("user", userObject && userObject, " end")
  useEffect(() => {
    if (userCookie) {
  ...
  const subjectCookie = cookie.get("subject")

  const [subjectObject, setSubjectObject] = useState(null)
  
  console.log("subject", subjectObject && subjectObject, " end")
  useEffect(() => {
    if (subjectCookie) {
      setSubjectObject(JSON.parse(subjectCookie))
  ...
  useEffect(() => {
    const getData = async () => {
      try {
          ...
          const response = await axios.post('/api/get_mock_test', { key: 'Vx0cbjkzfQpyTObY8vfqgN1us',  program_id: program_id ,subject_id:subjectObject?.id, test_type:"subject_wise"})
          console.log("REEEESSSS___PPP___NSE",response)
            ...
            <div className="text-black  flex justify-center items-center md:space-x-6 md:px-[2rem] " style={{ width: "100%" }}>
              ...
              <div className="w-full">
                ...
                {/* {check ? <>  <div className="rounded-md border border-[#FAD7DD] mt-3">
                  <div><p className="bg-[#FAD7DD38]  py-3 px-3 font-[500]"> {eachMcq ? eachMcq?.statistics_answer_percentage :mcqs[0]&& mcqs[0]?.statistics_answer_percentage} % people answer it correct</p></div>
